...
class Feed extends Component {
  ...
  render() {
    // Receive state from parent component via props DONE.
    // let feed = RECEIVE STATE FROM PARENT DONE
    let renderFeed = this.feed.map(article => {
...
class App extends Component {
  ...
  randomizer() {
    ...
    this.setState({randomFeed: this.state.randomFeed, feed: []},
    this.setState({loader: this.state.loader, loader: <Loader />}),
    // Set state. When the button is clicked, set the state for randomFeed and the loader.DONE

    componentDidMount() {
    let url = 'https://newsapi.org/v1/articles?source=the-next-web&sortBy=latest&apiKey=530fac4fc3af4d649eb52de85f02e1fb'
